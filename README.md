...
<head>
    ...
    <style>
        ...
        @media print {
            .sidebar, .btn, #apply-filter, #print-history {
...
<body>
                    ...
                    <form id="renter-form">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-6">
                                ...
                                <img id="id-card-preview" class="id-card-preview" alt="پێشبینینی وێنەی کارتی نیشتمانی">
                            </div>
                            <div class="col-md-6">
                                <label for="customer-search" class="form-label">گەڕان بەدوای کەسەکان</label>
                                <div class="search-box">
                                    <input type="text" class="form-control" id="customer-search" placeholder="ناوی کەس بنووسە...">
                ...
                <div class="card-body">
                    <form id="car-form">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label for="car-model" class="form-label">مۆدێلی سەیارە</label>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label for="car-image" class="form-label">وێنەی سەیارە</label>
                                <input type="file" class="form-control" id="car-image" accept="image/*">
                                <img id="car-image-preview" class="car-image-preview" alt="پێشبینینی وێنەی سەیارە">
                            </div>
                        </div>
    ...
    <script>
        ...
        let rentals = [];
        let cars = JSON.parse(localStorage.getItem('cars')) || sampleCars;
        ...
        document.querySelectorAll('.sidebar a').forEach(link => {
            link.addEventListener('click', function(e) {
                ...
                // ئەگەر بەشی مێژووی بەکرێدان هەڵبژێردرا، مێژووەکە نوێبکەرەوە
                if (sectionId === 'rental-history') {
                    updateRentalHistoryTable();
                } else if (sectionId === 'garage') {
                    updateCarList();
                }
            });
        });
        ...
        function displayCarResults(carsToShow) {
            const resultsContainer = document.getElementById('car-results');
            resultsContainer.innerHTML = '';
            
            if (carsToShow.length === 0) {
                resultsContainer.innerHTML = '<div class="col-12 text-center"><p>هیچ سەیارەیەک نەدۆزرایەوە</p></div>';
                return;
            }
        ...
        // پێشبینینی وێنەی کارتی نیشتمانی
        ...
        // زیادکردنی کەسی نوێ و تۆمارکردنی بەکرێدان
        document.getElementById('renter-form').addEventListener('submit', function(e) {
            ...
            if (name && phone && rentDate && returnDate && price) {
                // زیادکردنی کەسەکە بە لیستی کەسەکان
                const newCustomer = {
                ...
                // پشکنین بۆ بینینی ئەگەر کەسەکە هەیە یان نا
                const existingCustomer = customers.find(c => c.name === name && c.phone === phone);
                if (!existingCustomer) {
                    customers.push(newCustomer);
                    localStorage.setItem('customers', JSON.stringify(customers));
                }
                
                // زیادکردنی تۆمارەکەی بەکرێدان
                const newRental = {
                    id: rentalHistory.length > 0 ? Math.max(...rentalHistory.map(r => r.id)) + 1 : 1,
                    customerName: name,
                    carName: carSearch.value || 'سەیارەی هەڵنەبژێردراوە',
                    rentDate: rentDate,
        ...
        document.getElementById('car-form').addEventListener('submit', function(e) {
            ...
            if (name && model && color && plate && type) {
                const newCar = {
                    ...
                    plate: plate,
                    type: type,
                    image: document.getElementById('car-image-preview').src || 'https://via.placeholder.com/300x150?text=Car+Image'
                };
        ...
        // نوێکردنەوەی لیستی سەیارەکان
        function updateCarList() {
            const carListContainer = document.getElementById('car-list');
            carListContainer.innerHTML = '';
            
            if (cars.length === 0) {
                carListContainer.innerHTML = '<div class="col-12 text-center"><p>هیچ سەیارەیەک تۆمارنەکراوە</p></div>';
                return;
            }
                ...
                col.innerHTML = `
                    <div class="card">
                        ...
                        <div class="card-body">
                            ...
                            <button class="btn btn-danger btn-sm delete-car" data-car-id="${car.id}">سڕینەوە</button>
                        </div>
                    </div>
                `;
                carListContainer.appendChild(col);
            });
            
            // زیادکردنی ڕیەکسان بۆ سڕینەوەی سەیارە
            document.querySelectorAll('.delete-car').forEach(button => {
                button.addEventListener('click', function() {
                    const carId = parseInt(this.getAttribute('data-car-id'));
                    if (confirm('دڵنیای لە سڕینەوەی ئەم سەیارەیە؟')) {
        ...
        // نوێکردنەوەی خشتەی مێژووی بەکرێدان
        function updateRentalHistoryTable() {
            const tableBody = document.getElementById('rental-history-table');
            tableBody.innerHTML = '';
            
            if (rentalHistory.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="6" class="text-center">هیچ تۆمارێکی بەکرێدان نیە</td></tr>';
                return;
            }
            
            // فلترکردن بەپێی بەروار و ناوی کەس
            const startDate = document.getElementById('filter-start-date').value;
            const endDate = document.getElementById('filter-end-date').value;
            const customerFilter = document.getElementById('filter-customer').value.toLowerCase();
            
            let filteredRentals = rentalHistory;
            ...
            if (endDate) {
                filteredRentals = filteredRentals.filter(rental => rental.returnDate <= endDate);
            }
            
            if (customerFilter) {
                filteredRentals = filteredRentals.filter(rental => 
                    rental.customerName.toLowerCase().includes(customerFilter)
                );
            }
            
            if (filteredRentals.length === 0) {
                tableBody.innerHTML = '<tr><td colspan="6" class="text-center">هیچ تۆمارێکی بەکرێدان بەم فلترە نەدۆزرایەوە</td></tr>';
                return;
            }
            ...
            filteredRentals.forEach(rental => {
                ...
                row.innerHTML = `
                    ...
                    <td>${rental.price} دینار</td>
                    <td>
                        <button class="btn btn-danger btn-sm delete-rental" data-rental-id="${rental.id}">سڕینەوە</button>
                    </td>
                `;
                tableBody.appendChild(row);
            });
            
            // زیادکردنی ڕیەکسان بۆ سڕینەوەی تۆمارەکانی بەکرێدان
            document.querySelectorAll('.delete-rental').forEach(button => {
                button.addEventListener('click', function() {
                    const rentalId = parseInt(this.getAttribute('data-rental-id'));
                    if (confirm('دڵنیای لە سڕینەوەی ئەم تۆمارەی بەکرێدان؟')) {
                        rentalHistory = rentalHistory.filter(rental => rental.id !== rentalId);
                        localStorage.setItem('rentalHistory', JSON.stringify(rentalHistory));
                        updateRentalHistoryTable();
                    }
                });
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            // دانانی بەرواری ئەمڕۆ وەک بەرواری بنەڕەتی
            const today = new Date().toISOString().split('T')[0];
            document.getElementById('rent-date').value = today;
            
            // دانانی بەرواری سبەینێ وەک بەرواری وەرگرتنەوەی بنەڕەتی
